...
data Tm∘ (Σ : Sig) (Γ : Set) : Set where
    var∘ : Γ → Tm∘ Σ Γ
    sym∘ : (x : Sig.C Σ) → Vec (Tm∘ Σ Γ) (Sig.arity Σ x) → Tm∘ Σ Γ
...
recTm∘ M (sym∘ x 𝐭) = DepM∘.sym∘• M x 𝐭 (all (DepM∘.Tm∘• M) (recTm∘ M) _ 𝐭)
...
postulate
    ...
    sym : ∀ {Σ 𝓔} → (c : Sig.C Σ) → Vec (Tm Σ 𝓔) (Sig.arity Σ c) → Tm Σ 𝓔
...
_⟨_⟩ {Σ} {𝓔} {Γ} t σ = recTm∘ M t
    where   M : DepM∘ Σ Γ
            M = record {
                Tm∘•  = λ _ → Tm Σ 𝓔 ;
                var∘• = λ x → σ x ;
                sym∘• = λ x 𝐭 𝐭• → sym x (transp (All-const _ _) 𝐭•)
                }
    ...
    field
        ...
        eq• :   (e : EqTh.E 𝓔) →
                ...
                let M : DepM∘ Σ (EqTh.Ctx 𝓔 e)
                    -- we interpret the lhs and rhs according to the methods above
                    M = record {
                            Tm∘•  = λ t → Tm• (t ⟨ γₑ ⟩)
                        ;   var∘• = λ x → γₑ• x
                        ;   sym∘• = λ x 𝐭 𝐭• → transp (cong (λ X → Tm• (sym x X)) (all-map _ _ _))
                                                (sym• x (map (λ x → x ⟨ γₑ ⟩) _ 𝐭) ((transp (All-map 𝐭) 𝐭•)))
                        }
